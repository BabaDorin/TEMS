...
public class ChangeLogRepository(IMongoDatabase database) : IChangeLogRepository
{
    ...
    private readonly IMongoCollection<TicketChangeLogEntity> _ticketLogs =
        database.GetCollection<TicketChangeLogEntity>("ticket_change_logs");
    ...
    public async Task CreateAsync(ChangeLogEntry entry, CancellationToken cancellationToken = default)
    {
        switch (entry.EntityType)
        {
            ...
            case ChangeLogEntityType.Location:
                ...
                break;
            case ChangeLogEntityType.Ticket:
                await _ticketLogs.InsertOneAsync(TicketChangeLogMapper.ToDatabase(entry), cancellationToken: cancellationToken);
                break;
            default:
        ...
        CancellationToken cancellationToken = default)
    {
        return entityType switch
        {
            ...
            ChangeLogEntityType.Ticket => await GetTicketLogsAsync(entityId, tenantId, pageNumber, pageSize, cancellationToken),
    ...
    private async Task<(List<ChangeLogEntry> Entries, int TotalCount)> GetTicketLogsAsync(
        string entityId, string tenantId, int pageNumber, int pageSize, CancellationToken ct)
    {
        var filter = Builders<TicketChangeLogEntity>.Filter.And(
            Builders<TicketChangeLogEntity>.Filter.Eq(x => x.TicketId, entityId),
            Builders<TicketChangeLogEntity>.Filter.Eq(x => x.TenantId, tenantId)
        );

        var totalCount = await _ticketLogs.CountDocumentsAsync(filter, cancellationToken: ct);
        var entities = await _ticketLogs
            .Find(filter)
            .SortByDescending(x => x.Timestamp)
            .Skip((pageNumber - 1) * pageSize)
            .Limit(pageSize)
