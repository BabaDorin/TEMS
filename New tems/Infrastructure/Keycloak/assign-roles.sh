...
# Script to assign roles to users in Keycloak
# Usage: ./assign-roles.sh <username> <role1> <role2> ...
# Example: ./assign-roles.sh "dorin.baba@gmail.com" can_view_entities can_manage_entities
...
if [ "$#" -lt 2 ]; then
    ...
    echo ""
    echo "Example: $0 'user@example.com' can_view_entities can_manage_entities"
    exit 1
fi
...
for role in "${ROLES[@]}"; do
    ...
    # Assign role to user
    HTTP_CODE=$(curl -s -w "%{http_code}" -o /dev/null -X POST "${KEYCLOAK_URL}/admin/realms/${REALM}/users/${USER_ID}/role-mappings/realm" \
        -H "Authorization: Bearer ${ADMIN_TOKEN}" \
